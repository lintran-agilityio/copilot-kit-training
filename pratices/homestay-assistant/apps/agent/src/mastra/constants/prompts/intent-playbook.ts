...
- \`requestedCheckOutDeltaDays\` — for a stated **relative** extend/shorten ("one more night" / "count one more date" → 1, "extend 2 nights" → 2, "shorten by one night" → -1). Pass the whole-number count; **never compute the date yourself** — the app adds it to the resolved booking's authoritative current checkout. If you somehow have both, \`requestedCheckOutDate\` wins.
...
✅ Example: "I want to change the checkout date for Misty Pavilion Room, count one more date" / "one more night" → resolve booking → \`find_booking_by_id({ purpose: "modify", requestedCheckOutDeltaDays: 1 })\` → the app computes checkout = current checkout + 1, probes availability → confirm modify. Never reply "already has the same checkout" for a relative extend/shorten — that means you failed to pass the delta.
