...
export const getBookingsTool = createTool({
  ...
  description:
    "Get bookings from the backend. For personal bookings, omit userId — the signed-in user is resolved automatically from the server session. After calling: update_bookings_list with result.bookings → always finish with one short guest-facing chat sentence. List sync alone is not a complete reply.",
