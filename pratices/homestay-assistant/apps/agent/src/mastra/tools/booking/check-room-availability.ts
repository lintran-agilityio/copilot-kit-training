...
export const checkRoomAvailabilityTool = createTool({
  ...
  description:
    "Check whether a room is free for dates AND whether guests fit room.capacity. Always pass guests from the LATEST user message (not an earlier turn). Returns available, guestsWithinCapacity, and full room. If guestsWithinCapacity is false, call show_booking_unavailable (reason: capacity_exceeded, include room.capacity) — wait for the guest to close the modal, THEN reply in chat; do NOT call open_confirm_booking. If available is false for dates, call show_booking_unavailable (reason: dates_unavailable) — wait for modal close, THEN reply in chat; optionally getAvailableRooms. If available is true, call open_confirm_booking with result.room + dates + the same guests. Do not call getRoomById, show_room_detail, or open_room_detail_modal. Always call this before open_confirm_booking.",
