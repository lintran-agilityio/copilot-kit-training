...
export const checkRoomAvailabilityTool = createTool({
  ...
  description:
    "Check whether a room is free for dates AND whether guests fit room.capacity. Always pass absolute YYYY-MM-DD check-in/out resolved from CURRENT DATE / agent context today+tomorrow (never invent years like 2023). Always pass guests from the LATEST user message (not an earlier turn). Returns available, guestsWithinCapacity, and full room. If guestsWithinCapacity is false or available is false, do NOT call confirm_booking — BookingUnavailableModal renders automatically from this result; reply in chat explaining capacity or dates; optionally find_room with date when dates are taken. If available is true, call confirm_booking with result.room + dates + the same guests — wait for confirmed: true before create_booking. Do not call get_room_by_id in a book turn. Always call this before confirm_booking.",
