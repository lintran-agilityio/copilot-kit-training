  ...
  useHumanInTheLoop(
    {
      ...
      description:
        "After check_room_availability succeeds for a MODIFY flow (flow=modify + excludeBookingId), show the read-only confirm modification modal with before→after diffs. Pass bookingId, result.room, and the SAME checkInDate, checkOutDate, and guests from check_room_availability.result (the values just validated — from edit_modify_booking confirmed:true, or the stated change merged over the booking when the edit form was skipped). Also pass originalCheckInDate, originalCheckOutDate, and originalGuests — the pre-change booking values, taken from the edit_modify_booking tool-call args, or from find_booking_by_id / get_bookings when the form was skipped — so the UI can show only changed fields. Never put original booking dates into checkInDate/checkOutDate/guests. Do NOT call update_booking until confirm_modify_booking returns confirmed: true. If confirmed: true, call update_booking with bookingId, checkInDate, checkOutDate, and guests from the result — the UI shows ConfirmSuccess and refreshes the list automatically; then send one short guest-facing chat confirmation. If confirmed: false, reply that the booking was kept unchanged. Never use this for creating a new booking.",
