...
export const findRoomPurposeSchema = z
  ...
  .optional()
  .describe(
    'search = FIND/filter ("find/show Misty Pavilion") — Room List always. recommend = soft-book without a named room — Room List. book_resolve = BOOK name lookup ("I want to book Misty Pavilion for 1 guest this weekend") — pass name to match the room, PLUS date/guests when the latest message states them (never used to filter the name match — they seed the tool\'s own availability probe of the resolved room and let the platform decide the next step: skip the Booking Form and go to confirm_booking when both are known and the room is free, render BookingUnavailable when it is taken, open the form otherwise); skip Room List when exactly 1 match, show list when >1, and continue the booking flow with the matched room id. CREATE never uses check_room_availability. resolve = internal room-name → roomId lookup for cancel/modify without a bookingId (e.g. "modify guest number for Moonlight Loft") — pass name only; Room List always suppressed regardless of match count; use the single match\'s id as get_bookings.roomId, or ask which room when there is more than one match. Never use book_resolve or resolve for find/show/tell-me-about.',
  );
